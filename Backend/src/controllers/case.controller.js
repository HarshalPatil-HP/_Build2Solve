...
const getOne = asyncHandler(async (req, res) => {
  const doc = await caseService.getCaseById(req.params.id);
  res.status(200).json(new ApiResponse(200, doc));
});

const update = asyncHandler(async (req, res) => {
  const doc = await caseService.updateCase(req.params.id, req.body);
  res.status(200).json(new ApiResponse(200, doc, 'Case updated'));
});

const lock = asyncHandler(async (req, res) => {
  const doc = await caseService.lockCase(req.params.id);
  res.status(200).json(new ApiResponse(200, doc, 'Case locked'));
});

const addendum = asyncHandler(async (req, res) => {
  const doc = await caseService.addAddendum(req.params.id, { ...req.body, inspectorId: req.user.userId });
